...
export const MalteadasPage = () => {
    ...
    const malteadasConDescuento = malteadas.map(m => {
    ...
    return (
        <Box>
            ...
            <Grid container spacing={4}>
                {malteadasConDescuento.map(item => (
                    <Grid item xs={12} sm={6} md={4} key={item.id}>
                        <ProductCard
                            ...
                            details={
                              isAuthenticated
                                ? `${item.details}\nDescuento: ${item.descuento}%\nPrecio original: $${item.price}\nPrecio con descuento: $${item.precioConDescuento}`
                                : item.details
                            }
                            price={isAuthenticated ? item.precioConDescuento : item.price}
                        />
                    </Grid>
                ))}
            </Grid>
